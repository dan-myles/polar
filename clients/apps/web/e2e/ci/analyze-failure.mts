...
Each test drives a real Chrome with Stagehand. It creates the product it needs through the API if it does not exist yet, creates a checkout for it and opens it, then fills the form like a customer: email, and for paid products the cardholder name, a US billing address (country, street, postal code, city, state) and Stripe's 4242 test card typed into the Payment Element iframe. It clicks the button that completes the checkout, expects a redirect to /confirmation, polls the Polar API until the checkout status is "succeeded" (for paid products this depends on Stripe's webhook reaching Polar), then checks the resulting order or subscription through the customer portal API and cancels any subscription at the end of its period.
