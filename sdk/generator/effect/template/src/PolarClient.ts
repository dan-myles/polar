...
import { Context, Data, DateTime, Duration, Effect, Layer, Option, Redacted, Schema, Stream } from "effect";
...
export const withAccessToken: {
  (accessToken: Redacted.Redacted<string> | string): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E, R | PolarClient>;
  <A, E, R>(self: Effect.Effect<A, E, R>, accessToken: Redacted.Redacted<string> | string): Effect.Effect<A, E, R | PolarClient>;
} = ((...args: ReadonlyArray<unknown>) => {
  if (args.length === 1) {
    return (self: Effect.Effect<unknown, unknown, unknown>) => withAccessToken(self, args[0] as string);
  }
  const [self, accessToken] = args as [Effect.Effect<unknown, unknown, unknown>, Redacted.Redacted<string> | string];
...
type QueryParams = Readonly<Record<string, QueryValue | ReadonlyArray<QueryValue> | Readonly<Record<string, unknown>>>>;
...
const parseRetryAfter = (response: HttpClientResponse.HttpClientResponse): Option.Option<Duration.Duration> => {
  ...
  return Number.isNaN(date) ? Option.none() : Option.some(Duration.millis(Math.max(0, date - Date.now())));
};
...
export const request = <S extends Schema.Top | "text" | "none", E = never>(
  ...
  Effect.gen(function* () {
    ...
    if (spec.body !== undefined) {
      ...
      const encoded = yield* Schema.encodeUnknownEffect(schema as Schema.Codec<unknown, unknown>)(value);
      httpRequest = HttpClientRequest.bodyJsonUnsafe(httpRequest, encoded);
    }

    const retryable = (status: number) => status === 429 || (status >= 500 && isIdempotent(spec.method));

    let attempt = 0;
    while (true) {
      const result = yield* client.httpClient.execute(httpRequest).pipe(
        Effect.timeout(client.timeout),
        Effect.result,
      );
      ...
      if (status >= 200 && status < 300) {
        ...
        return (yield* HttpClientResponse.schemaBodyJson(spec.success as Schema.Codec<unknown, unknown>)(
          response,
        )) as SuccessType<S>;
      }
...
export const paginate = <A, E, R>(
  ...
  Stream.paginate(startPage, (page) =>
    Effect.map(fetchPage(page), (response) => [
      response.items,
...
): Layer.Layer<PolarClient, never, HttpClient.HttpClient> => Layer.effect(PolarClient, make(options));
